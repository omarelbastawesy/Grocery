      ...
      <div className="flex flex-col flex-1 w-full gap-3">
        ...
        <div className="flex items-center justify-between mt-auto pt-4 border-t border-slate-100">
          <div className="flex flex-col items-start leading-[1.1]">
            <p className="font-medium line-through text-slate-400 text-sm">
              {final_price && price}
            </p>
            <p className="font-bold text-[#014162] text-[20px]">
              {final_price || price}
            </p>
          </div>
          <button
            disabled={!in_stock}
            onClick={() => mutate({ meal_id: link, quantity: 1 })}
            className={`${in_stock ? "cursor-pointer" : "cursor-not-allowed"} flex shrink-0 items-center gap-1.5 justify-center bg-[#014162]/5 hover:bg-[#014162] text-[#014162] hover:text-white transition-all duration-300 h-9 px-4 relative rounded-lg font-semibold text-sm ${!in_stock ? "opacity-50 cursor-not-allowed" : ""}`}
          >
            <ShoppingCart size={16} />
            <span>{in_stock ? "Add" : "Out of Stock"}</span>
          </button>
        </div>
      </div>
    </div>
